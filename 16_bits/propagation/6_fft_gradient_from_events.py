...
Same as 4_fft_cyl_gradient.py, but additionally reads the individual event/sec input images
and produces an extra "FROM_EVENTS" plot: CERN-01_event, CERN-02_sec, combined phase, and CCD intensity.
...
FILENAME_CERN02_SEC = "16_bits/propagation/img/input/SLM_16x16bits_CERN-02_sec3_960x960_r16_c16_w108.bmp"
FILENAME_COMBINED = "16_bits/propagation/img/input/SLM_16x16bits_CERN-01_event3_CERN-02_sec3_960x960_r16_c16_w108.bmp"
...
def main() -> None:
    ...
    # --- Load additional input files for FROM_EVENTS plot ---
    data_cern01_bw = None
    data_cern02_bw = None
    data_combined_bw = None
    path_cern01 = _resolve_path(FILENAME_CERN01_EVENT)
    path_cern02 = _resolve_path(FILENAME_CERN02_SEC)
    path_combined = _resolve_path(FILENAME_COMBINED)
    if path_cern01 is not None:
        print(f"Loading (CERN-01 event): {path_cern01}")
        data_cern01_bw = _as_bw_0_255(_load_grayscale_u8(path_cern01))
    else:
        print(f"WARNING: File not found: {FILENAME_CERN01_EVENT}")
    if path_cern02 is not None:
        print(f"Loading (CERN-02 sec):   {path_cern02}")
        data_cern02_bw = _as_bw_0_255(_load_grayscale_u8(path_cern02))
    else:
        print(f"WARNING: File not found: {FILENAME_CERN02_SEC}")
    if path_combined is not None:
        print(f"Loading (combined):      {path_combined}")
        data_combined_bw = _as_bw_0_255(_load_grayscale_u8(path_combined))
    else:
        print(f"WARNING: File not found: {FILENAME_COMBINED}; using data_orig_bw for combined.")
        data_combined_bw = data_orig_bw
    ...
    if SAVE_PLOTS or SHOW_FIGURE:
        ...
        # FROM_EVENTS: CERN-01_event + CERN-02_sec (B&W) | combined (B&W) | CCD intensity (linear)
        can_make_from_events = (
            data_cern01_bw is not None
            and data_cern02_bw is not None
        ...
        if can_make_from_events:
            ...
            fig4.tight_layout()
            if SAVE_PLOTS:
                out_fig4 = out_dir / f"{stem}_FFT_cylindrical_lens_FROM_EVENTS.png"
                fig4.savefig(str(out_fig4), dpi=220, bbox_inches="tight")
